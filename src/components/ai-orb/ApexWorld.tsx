...
/* Mirrors the ROSTER in ReasoningWeb.jsx (a verbatim copy from the Apex app, so
   ...
   the copy's roster changes. Lives in ./roster (plain data, no "use client") so
   Server Components — e.g. this page's metadata — can read it too. */
export { ROSTER };
...
export const INFO: Record<string, AgentInfo> = {
  ...
  engineering: { role: "Engineering brain", status: "online",
    caps: ["Technical scoping and architecture calls", "Code review and API integrations", "Deployment and bug triage"],
    asks: ["Scope this feature", "Review this integration"] },
  ...
  developer: { role: "Keeper of the build log", status: "standby",
    caps: ["Keeps Ama's development log", "Recaps what shipped - day / week / month", "Future: extends Ama itself"],
  ...
  product: { role: "Roadmap and scope", status: "online",
    caps: ["Turns requests into scoped, buildable features", "Prioritizes the roadmap against real demand", "Keeps releases tied to client outcomes"],
    asks: ["Scope this request", "What ships next?"] },
  security: { role: "Risk and access control", status: "online",
    caps: ["Reviews new integrations before they ship", "Flags data-handling and access risks", "Keeps client data encrypted and scoped correctly"],
    asks: ["Is this integration safe?", "Review our access controls"] },
  legal: { role: "Contracts and compliance", status: "online",
    caps: ["Drafts and reviews client agreements", "Flags compliance gaps early", "Keeps data-handling terms current"],
    asks: ["Review this contract", "Are we compliant here?"] },
  devops: { role: "Infrastructure and uptime", status: "online",
    caps: ["Deployment pipelines and monitoring", "Uptime and incident response", "Scales infrastructure with demand"],
    asks: ["Is production healthy?", "Scale this for launch"] },
  qa: { role: "Quality before it ships", status: "online",
    caps: ["Tests every release before go-live", "Catches regressions early", "Keeps a clean bug backlog"],
    asks: ["Test this release", "What's still open?"] },
  customer_success: { role: "Keeps clients live and happy", status: "online",
    caps: ["Onboards new clients end-to-end", "Catches churn risk early", "Turns support tickets into product feedback"],
    asks: ["How's this client doing?", "Draft an onboarding plan"] },
  dataml: { role: "Data and automation", status: "online",
    caps: ["Builds the models Ama runs on", "Data pipelines and reporting", "Finds where automation pays off"],
    asks: ["What does the data say?", "Where can we automate this?"] },
  bizdev: { role: "Partnerships and growth", status: "online",
    caps: ["Sources new partnerships and channels", "Scopes enterprise deals", "Opens new markets"],
    asks: ["Find us a partner here", "Scope this enterprise deal"] },
  hr: { role: "People operations", status: "integration",
    caps: ["Tracks open roles and hiring pipeline", "Onboarding checklists for new hires"] },
};

const STATUS_LINE: Record<AgentInfo["status"], { color: string; text: string }> = {
  online: { color: "#34d399", text: "Online - Ama routes work to it automatically" },
...
export function AgentOverview({ sel, onClose }: { sel: NodeSel; onClose: () => void }) {
  ...
  const info = INFO[sel.key] ?? { role: "Specialist", status: "online" as const, caps: ["Part of the Ama core"] };
  const c = sel.color;
...
export default function ApexWorld({
  ...
  return (
    <div style={{ position: "absolute", inset: 0, overflow: "hidden", userSelect: "none" }}>
      ...
      <nav className="visually-hidden" aria-label="Ama agents">
        <ul>
          {ROSTER.map((a) => (
            <li key={a.key}>
